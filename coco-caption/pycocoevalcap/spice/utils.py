...
class ConllEntry:
    ...
    def __init__(self, id, form, parent_id=None, relation=None, prop=None):
        self.id = id
        self.form = form
    ...
    def __str__(self):
        values = [str(self.id), self.form, str(self.pred_parent_id) if self.pred_parent_id is not None else None, self.pred_relation]
...
def vocab(conll_path):
    ...
    return (wordsCount, {w: i for i, w in enumerate(wordsCount.keys())}, relCount.keys())


def read_conll(fh, proj):
    #fout = codecs.open('non_project.conll', 'w', encoding='utf-8')
    
    dropped = 0
    read = 0
    #root = ConllEntry(0, '*root*', '*root*', 'ROOT-POS', 'ROOT#-CPOS', '_', -1, 'rroot', '_', '_')
    root = ConllEntry(0, '*root*', -1, 'rroot', 'ROOT')
    tokens = [root]
    #print fh
    for line in fh:
        #print 'conll line:, ', line
        ...
        if not tok or line.strip() == '':
            if len(tokens)>1:
                ...
                if not proj or isProj([t for t in tokens if (isinstance(t, ConllEntry) and (t.parent_id != -1 or t.id == 0) )]):
            ...
            if line[0] == '#' or '-' in tok[0] or '.' in tok[0]:
                ...
                tokens.append(ConllEntry(int(tok[0]), tok[1], int(tok[2]) if tok[2] != '_' else -1, tok[3], tok[4]))        
    if len(tokens) > 1:
        yield tokens
